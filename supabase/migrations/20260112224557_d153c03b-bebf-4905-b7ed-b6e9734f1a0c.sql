-- Create atomic function to check and increment session count in a single transaction
-- This prevents race conditions where users could open multiple tabs to bypass limits

CREATE OR REPLACE FUNCTION public.atomic_check_and_increment_session(
  p_email TEXT,
  p_session_type TEXT
)
RETURNS JSONB
...
GRANT EXECUTE ON FUNCTION public.atomic_check_and_increment_session(TEXT, TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION public.atomic_check_and_increment_session(TEXT, TEXT) TO service_role;

-- Add comment explaining the function
COMMENT ON FUNCTION public.atomic_check_and_increment_session IS 
'Atomically checks if a Pro user can start a session and increments the count in a single transaction.
Uses SELECT FOR UPDATE to prevent race conditions from multiple simultaneous session starts.';